...
// Create a flat array of all items for the "all" category
...
export const mockItems = {
  all: allItems,
  "Fruits & Vegetables": allItems.filter((item) =>
    [
      "Fresh Tomatoes",
      "Organic Bananas",
      "Oranges",
  ...
  "Dairy & Breakfast": allItems.filter((item) =>
    [
      "Fresh Milk",
      "Whole Wheat Bread",
      "Cottage Cheese",
  ...
  "Snacks & Beverages": allItems.filter((item) =>
    ["Potato Chips", "Chocolate Bars", "Pretzels", "Mixed Nuts"].includes(
      item.name
    )
  ),
  "Household Essentials": allItems.filter((item) =>
    ["Dish Soap", "Laundry Detergent", "Paper Towels", "Trash Bags"].includes(
      item.name
    )
  ),
  "Personal Care": allItems.filter((item) =>
    ["Hand Sanitizer", "Shampoo", "Body Wash", "Toothpaste"].includes(item.name)
  ),
  Beverages: allItems.filter((item) =>
    ["Cola", "Coffee", "Green Tea", "Orange Juice"].includes(item.name)
  ),
  Bakery: allItems.filter((item) =>
    ["Croissant", "Chocolate Cake", "Bagels", "Muffins"].includes(item.name)
  ),
  "Frozen Foods": allItems.filter((item) =>
    ["Frozen Pizza", "Ice Cream", "Frozen Vegetables", "Frozen Fish"].includes(
      item.name
    )
  ),
};
